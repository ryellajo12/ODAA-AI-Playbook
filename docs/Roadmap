...
block-beta
    ...
    block:channels:1
        columns 6
        A["Oracle Digital Assistant"] B["Slack"] C["Microsoft Teams"] D["Facebook Messenger"] E["Web Chat"] F["Webhooks / APIs"]
    end
    ...
    block:agents:1
        columns 4
        G["HR & Workforce Agent"] H["Finance & Procurement Agent"] I["Supply Chain Agent"] J["Campus Solutions Agent"]
    end
    ...
    block:tech:1
        columns 4
        K["PeopleSoft"] L["Oracle Fusion Cloud (HCM/ERP/SCM)"] M["Oracle Autonomous Database"] N["Oracle Integration Cloud"]
    end
    ...
    block:ai:1
        columns 3
        O["OCI Generative AI Agents (RAG · SQL · Function Calling · API Endpoint · Agent-as-a-Tool)"] P["OCI Generative AI LLMs (Cohere · Llama · Grok)"] Q["OCI AI Services (Vision · Speech · Language)"]
    end
    ...
    block:dev:1
        columns 3
        R["Agent Development Kit (ADK) Python & Java"] S["OCI Console Agent Builder"] T["CLI / REST APIs"]
    end
    ...
    block:infra:1
        columns 5
        U["Compute & Containers"] V["Networking (VCN)"] W["Object Storage"] X["IAM (Identity Domains)"] Y["Vault (Key Mgmt)"]
    end
    ...
    style dev fill:#F3E5F5,stroke:#6A1B9A
    style infra fill:#ECEFF1,stroke:#455A64
```
...
block-beta
    ...
    block:channels:1
        columns 5
        A["Microsoft Teams"] B["Copilot Studio"] C["SharePoint Online"] D["M365 Copilot"] E["Web Portal"]
    end
    ...
    block:agents:1
        columns 4
        F["HR & Workforce Agent"] G["Finance & Procurement Agent"] H["Supply Chain Agent"] I["Compliance & Governance Agent"]
    end
    ...
    block:ai:1
        columns 4
        J["Azure OpenAI"] K["Microsoft Foundry"] L["Foundry IQ (Search & RAG)"] M["Copilot SDK"]
    end
    ...
    block:iq:1
        columns 3
        IQ1["Work IQ (M365 Context · Memory · Inference)"] IQ2["Foundry IQ (Vector · Hybrid · Semantic Search)"] IQ3["Fabric IQ (Data Intelligence · Analytics · OneLake)"]
    end
    ...
    block:enterprise:1
        columns 4
        N["Microsoft Entra ID"] O["Microsoft Purview"] P["Microsoft Graph API"] Q["SharePoint & Teams Data"]
    end
    ...
    block:infra:1
        columns 4
        R["AKS / App Service"] S["Azure Functions"] T["Cosmos DB"] U["Key Vault"]
    end
    ...
    style agents fill:#E8F5E9,stroke:#2E7D32
    style ai fill:#FCE4EC,stroke:#C62828
    style iq fill:#EDE7F6,stroke:#4527A0
...
block-beta
    ...
    block:surfaces:1
        columns 3
        A["Microsoft Teams"] B["Copilot Studio"] C["Web Portal"]
    end
    ...
    block:unified:1
        columns 4
        D["HR & Workforce Agent (PeopleSoft + M365)"] E["Finance Agent (Oracle ERP + Purview)"] F["Supply Chain Agent (OCI + Azure)"] G["Governance Agent (Entra + Purview + OCI IAM)"]
    end
    ...
    block:iqblock:1
        columns 3
        IQ1["Work IQ (M365 Context · Memory · Inference)"] IQ2["Foundry IQ (Search · RAG · Embeddings)"] IQ3["Fabric IQ (Data Intelligence · OneLake)"]
    end

    space

    block:msEnterprise:1
        columns 4
        H["Microsoft Entra ID"] I["Microsoft Purview"] J["Microsoft Graph API"] K["SharePoint & Teams Data"]
    end
    ...
    block:interconnect:1
        columns 3
        L["Oracle Database Service for Azure (ODSA / Interconnect)"] M["Cross-Cloud Identity Federation"] N["Azure API Management"]
    end
    ...
    block:oci:1
        columns 4
        O["PeopleSoft"] P["Oracle Fusion Cloud (HCM/ERP/SCM)"] Q["Oracle Autonomous Database"] R["OCI Generative AI Agents"]
    end

    block:azure:1
        columns 4
        S["Azure OpenAI"] T["Microsoft Foundry"] U["Foundry IQ"] V["AKS / Functions / Cosmos DB"]
    end

    surfaces --> unified
    unified --> iqblock
    iqblock --> msEnterprise
    msEnterprise --> interconnect
    interconnect --> oci
    interconnect --> azure
    unified --> azure
    H --> surfaces

    style surfaces fill:#E3F2FD,stroke:#1565C0
    style unified fill:#E8F5E9,stroke:#2E7D32
    style iqblock fill:#EDE7F6,stroke:#4527A0
    style msEnterprise fill:#FFF3E0,stroke:#E65100
    style interconnect fill:#FFF9C4,stroke:#F57F17
    style oci fill:#FCE4EC,stroke:#C62828
    style azure fill:#E1F5FE,stroke:#0277BD
```
